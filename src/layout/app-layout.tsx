...
import { getMarkas } from 'apiSdk/markas';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['brand-owner', 'project-developer', 'marketing-manager', 'app-administrator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<MarkaInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/markas` : null),
    () => getMarkas({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Markas',
      path: isTenantUser ? `/markas/view/${data?.[0]?.id}` : '/markas',
      entity: 'marka',
