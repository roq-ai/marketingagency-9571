...
import { getSubmissions } from 'apiSdk/submissions';
...
import { SubmissionListPage as ShowCaseList } from 'pages/submissions';

function DashboardPage() {
  const { session } = useSession();

  const submissionStat = useSWR<PaginatedInterface<SubmissionInterface>>(
    () => `/submissions`,
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Submissions',
      result: submissionStat?.data?.totalCount,
      isLoading: submissionStat.isLoading,
    },
    {
      title: 'Projects',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`submissions`} passHref legacyBehavior>
                  <Button
