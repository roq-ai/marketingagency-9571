...
export function ProjectListPage(props: ProjectListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getProjects({
        relations: ['marka', 'user', 'submission.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('marka', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'marka',
          header: 'Marka',
          accessorKey: 'marka',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/markas/view/${record.marka?.id}`}>
              {record.marka?.name}
            </Link>
          ),
        }
    ...
    hasAccess('submission', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.submission,
        }
      : null,
