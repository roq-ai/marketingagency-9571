...
type ColumnType = ColumnDef<SubmissionInterface, unknown>;
...
export function SubmissionListPage(props: SubmissionListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getSubmissions({
        relations: ['project', 'user', 'review.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['status.contains', 'feedback.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SubmissionInterface>>(
    () => `/submissions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SubmissionInterface) => {
    if (hasAccess('submission', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/submissions/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/submissions/view/${record.id}`} passHref legacyBehavior>
                <Button
