...
import { getMarkas, deleteMarkaById } from 'apiSdk/markas';
import { MarkaInterface } from 'interfaces/marka';

type ColumnType = ColumnDef<MarkaInterface, unknown>;

interface MarkaListPageProps {
...
export function MarkaListPage(props: MarkaListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MarkaInterface>>(
    () => `/markas?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MarkaInterface) => {
    if (hasAccess('marka', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/markas/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/markas/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('marka', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/markas/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('marka', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
